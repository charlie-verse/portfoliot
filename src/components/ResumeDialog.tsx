...
const ResumeDialog: React.FC<ResumeDialogProps> = ({ isOpen, onClose }) => {
  ...
  const handleDownload = () => {
    ...
    link.download = "Kushal_C_Resume.pdf";
  ...
  const resumeData = {
    name: "Kushal C",
    title: "Software Developer | Open Source Contributor | Freelancer",
    // email: "fardeenmansoori26@gmail.com",
    // phone: "+91 9876543210",
    // linkedin: "linkedin.com/in/fardeenmansoori",
    // github: "github.com/Fardeen26",
    summary:
      "Software Developer with 1+ years building AI applications using *LangChain*, LLMs, specializing in *GenAI*, Web3 and *AI Agents*. Strong background in *cloud deployment*, optimization, and delivering scalable applications.",
    experience: [
      {
          ...
          >
            <div className="bg-white dark:bg-black border dark:border-white/20 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
              ...
              <div className="flex items-center justify-between p-6 border-b dark:border-white/20">
                <h2 className={`text-2xl font-semibold ${bricolage_grotesque}`}>
                  Resume Preview
                </h2>
              ...
              <div className="overflow-y-auto max-h-[calc(90vh-88px)] p-8">
                <div className="max-w-3xl mx-auto">
                  ...
                  <div className="text-center mb-8">
                    <h1
                      className={`text-3xl font-bold mb-2 ${bricolage_grotesque}`}
                    >
                      {resumeData.name}
                    </h1>
                    <p
                      className={`text-xl text-gray-600 dark:text-gray-400 mb-4 ${inter}`}
                    >
                      {resumeData.title}
                    </p>
                    <div
                      className={`flex flex-wrap justify-center gap-4 text-sm text-gray-600 dark:text-gray-400 ${inter}`}
                    >
                    ...
                    <div
                      className={`flex flex-wrap justify-center gap-4 text-sm text-gray-600 dark:text-gray-400 mt-2 ${inter}`}
                    >
                  ...
                  <div className="mb-8">
                    <h2
                      className={`text-xl font-semibold mb-3 ${bricolage_grotesque}`}
                    >
                      Professional Summary
                    </h2>
                    <p
                      className={`text-gray-600 dark:text-gray-400 leading-relaxed ${inter}`}
                    >
                  ...
                  <div className="mb-8">
                    <h2
                      className={`text-xl font-semibold mb-4 ${bricolage_grotesque}`}
                    >
                      Experience
                    </h2>
                    {resumeData.experience.map((exp, idx) => (
                      <div key={idx} className="mb-6">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h3
                              className={`text-lg font-semibold ${bricolage_grotesque}`}
                            >
                              {exp.title}
                            </h3>
                            <p
                              className={`text-gray-600 dark:text-gray-400 ${inter}`}
                            >
                              {exp.company}
                            </p>
                          </div>
                          <span
                            className={`text-sm text-gray-500 dark:text-gray-400 ${inter}`}
                          >
                            {exp.period}
                          </span>
                        </div>
                        <ul className="list-disc list-inside space-y-1 ml-4">
                          {exp.bullets.map((bullet, bulletIdx) => (
                            <li
                              key={bulletIdx}
                              className={`text-gray-600 dark:text-gray-400 text-sm leading-relaxed ${inter}`}
                            >
